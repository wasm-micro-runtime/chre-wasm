...
SHARED_ROOT := $(CHRE_PREFIX)/wamr/core/shared
...
COMMON_CFLAGS += -I$(IWASM_ROOT)/aot
...
COMMON_CFLAGS += -I$(IWASM_ROOT)/libraries/debug-engine
...
COMMON_CFLAGS += -I$(IWASM_ROOT)/libraries/libc-wasi/sandboxed-system-primitives/src
COMMON_CFLAGS += -I$(IWASM_ROOT)/libraries/libc-wasi/sandboxed-system-primitives/include
...
CFLAGS += -I${CORE_ROOT} \
          ...
          -I${SHARED_ROOT}/platform/linux

COMMON_CFLAGS += -I${CORE_ROOT} \
          -I${IWASM_ROOT}/include \
          -I${IWASM_ROOT}/interpreter \
          -I${IWASM_ROOT}/common \
          -I${IWASM_ROOT}/libraries/thread-mgr \
...
COMMON_CFLAGS += -I$(IWASM_ROOT)/interpreter
endif

CSRCS += platform_init.c \
         posix_thread.c \
         posix_time.c \
         posix_malloc.c \
         posix_memmap.c \
...
VPATH += $(SHARED_ROOT)/platform/linux
...
#include $(CHRE_PREFIX)/app_framework/app_framework.mk

#need to modify
OUTPUT_DIR := $(CHRE_PREFIX)/out/wamr_out

COBJS := $(patsubst %.c, $(OUTPUT_DIR)/%.o, $(CSRCS))
#COBJS := $(CSRCS:%.c=%.o)
LIBVMLIB = $(OUTPUT_DIR)/libvmlib.a
WASM_CALL_NATIVE := $(OUTPUT_DIR)/wasm_call_native.o
CFLAGS += -fPIC
#$(error $(COBJS))
$(LIBVMLIB):$(COBJS) $(WASM_CALL_NATIVE)
...
$(WASM_CALL_NATIVE):$(ASRCS)
	$(V)mkdir -p $(OUTPUT_DIR)
	$(V)echo " [AS] $@" 
	$(V)$(AS) -o $@ $^ $(AFLAGS)

COMMON_SRCS += $(LIBVMLIB)
